# econometric_data/econometric_modes/lasso_regression_model.py
...
def create_widgets(column_names, date_columns):
    """
    Create interactive widgets for selecting Y, Categorical X, and Continuous X variables for Lasso Regression.
    """
    y_variable_text = widgets.HTML(value=f"<b>Selecting variables for <span style='color:#228B22'>Lasso Regression</span> model:</b>")
    ...
    # Updated text with color highlights
    ...
    # Slider for selecting the alpha value
...
def confirm_selections(b, widgets_dict, all_columns, date_columns):
    ...
    alpha_value = widgets_dict["alpha_slider"].value
    ...
    if y_variable and (x_categorical_variables or x_continuous_variables):
        with variable_selection_output:
            ...
            variable_selection_output.append_display_data(
                widgets.HTML(
                    value=f"<b style='color:#FFD700;'>Alpha:</b> <span style='color:white;'>{alpha_value}</span>"
                )
            )
...
def display_widgets():
    """
    Main function to display the widgets for Lasso Regression variable selection.
    """
